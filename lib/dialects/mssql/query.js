      ...
      if (_.startsWith(self.sql, 'BEGIN TRANSACTION')) {
        ...
        var request = new connection.lib.Request(self.sql, function(err) {
          ...
          if (err) {
            ...
            if (!self.options.resultStream) resolve(self.formatResults(results));
            else resolve();
          }
          if (self.options.resultStream) self.options.resultStream.end();
        });

        if (self.options.resultStream) request.on('row', columns => self.options.resultStream.write(getRowObj(columns)) );
        else                           request.on('row', columns => results.push(getRowObj(columns)) );
