        ...
        if (self.sql.indexOf('-- ') === 0) {
          ...
          resolve(new Promise(function(resolve, reject) {
            ...
            if (method === 'exec') {
              ...
              if (self.options.resultStream && method === 'each') {
            	  self.database.each(self.sql,
                                     parameters,
                                     collectStreamData(self.options.resultStream),
                                     endStreamData(self.options.resultStream, resolve));
              } else {
...
Query.prototype.getDatabaseMethod = function() {
  if (this.isUpsertQuery()) {
    ...
    return 'run';
  } else if (this.options.resultStream) {
    return 'each';
  } else {
    return 'all';
  }
...
function collectStreamData(stream){return (err,row)=>{
	if (!err) stream.write(row);
};}

function endStreamData(stream, callback){return ()=>{
	stream.end();
	callback();
};}
